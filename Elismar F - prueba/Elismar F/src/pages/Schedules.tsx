...
export const Schedules: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!profile || !formData.file) return;
  ...
  const handleDelete = async () => {
    ...
    try {
      const filePath = scheduleToDelete.file_url.split('/documents/')[1];
      if (filePath) {
  ...
  return (
    <Layout>
          ...
          {schedules.length === 0 ? (
            ...
            schedules.map((schedule) => (
              ...
              >
                ...
                <h3 className="text-lg font-bold text-gray-900 mb-2">{schedule.title}</h3>
                <p className="text-sm text-gray-500 mb-4">
                  Subido por {schedule.profiles?.full_name || 'Desconocido'} <br />
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="Ej: Horario Primaria 2024"
      ...
      <ConfirmDialog
        ...
        message={`¿Estás seguro de que deseas eliminar el horario "${scheduleToDelete?.title}"? Esta acción no se puede deshacer.`}
