...
import { FileUp, BookOpen, CheckCircle, Trash2, Download, AlertCircle, Loader2, GraduationCap, ClipboardList, Layers } from 'lucide-react';
import { supabase, PlanesEvaluacion, getPublicUrl } from '../lib/supabase';
...
export const EvaluationPlans: React.FC = () => {
  ...
  const handleDelete = async (id: string, ruta_pdf: string) => {
    if (!confirm('¿Está seguro de eliminar este plan de evaluación?')) return;

    try {
      const filePath = ruta_pdf;
      if (filePath) {
        ...
        {canEdit && (
          <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 mb-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              ...
              <div className="space-y-2">
                <label className="block text-sm font-bold text-gray-700 flex items-center gap-1">
                  <BookOpen size={16} className="text-gray-400" /> Materia *
                </label>
                <input
                  type="text"
                  placeholder="Ej: Matemática III, GHC..."
                  className="w-full border border-gray-200 rounded-xl p-3 bg-gray-50 focus:ring-2 focus:ring-blue-500 focus:bg-white outline-none transition-all"
                  value={nombreMateria}
                    ...
                    <tr key={plan.id} className="group hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-8 py-6 text-right">
                        <div className="flex items-center justify-end gap-3">
                          <a
                            href={getPublicUrl('documentos_pdf', plan.ruta_pdf)}
                            target="_blank"
                          ...
                          {canEdit && (
                            <button
                              onClick={() => handleDelete(plan.id, plan.ruta_pdf)}
