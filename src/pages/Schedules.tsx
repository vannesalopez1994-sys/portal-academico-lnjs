...
import { FileUp, CalendarCheck2, CheckCircle, Trash2, Download, AlertCircle, Loader2, TableProperties, ListChecks, LayoutGrid, Eye } from 'lucide-react';
                      ...
                      <td className="px-8 py-6 text-right">
                        <div className="flex items-center justify-end gap-3">
                          <button
                            onClick={() => {
                              setSelectedPdf({
                                url: schedule.ruta_pdf,
                                name: `Horario ${schedule.anio_escolar} - Sección ${schedule.seccion}`
                              });
          ...
          {filteredSchedules.length > 0 && totalPages > 1 && (
            <div className="bg-white rounded-2xl shadow-sm border border-blue-100/60 p-4 px-6 flex flex-col sm:flex-row items-center justify-between gap-3 mt-4">
              <p className="text-xs text-gray-500 font-medium">
                Mostrando <span className="font-bold text-gray-800">{(currentPage - 1) * itemsPerPage + 1}–{Math.min(currentPage * itemsPerPage, filteredSchedules.length)}</span> de <span className="font-bold text-gray-800">{filteredSchedules.length}</span> horarios
              </p>
              <div className="flex items-center gap-2">
                <button
                  onClick={() => {
